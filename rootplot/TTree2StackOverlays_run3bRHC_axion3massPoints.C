...
#include "LoadSamples_FHCRuns_Run3.h"

#include "CommonCut.C"

void TTree2StackOverlays_run3bRHC_axion3massPoints(){
	...
	TString Label = "NuMIaxionsRun3bPre16880"+axiontag+"_2s0t_twoMassPoints";
...
//	Samples axion003		= LoadAxions003	(axiontag);
	...
	Samples Onepi0		= LoadRun3RHCOnepi0 (axiontag);
	Samples NueCC		= LoadRun3RHCNueCC (axiontag);
	Samples NumuCC		= LoadRun3RHCNumuCC (axiontag);
	Samples InCryoOther = LoadRun3RHCInCryoOther (axiontag);

	Samples dirt		= LoadRun3RHCDirt		(axiontag);

	Samples data		= LoadRun3Pre16880RHCData	(axiontag);


	double PlotPOT = 1e21;//data.GetPOT();
