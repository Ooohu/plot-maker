...
//#include "LoadSamples_newCatRun2b.h"
#include "LoadSamples_newCatRun2b_newMC.h"
...
void TTree2StackOverlays_run2b_AllMass_twoPoints(){
...
//	Samples ALL = LoadRun2bAll(axiontag);

	Samples Onepi0		= LoadRun2bOnepi0 (axiontag);
	Samples NueCC		= LoadRun2bNueCC (axiontag);
	Samples NumuCC		= LoadRun2bNumuCC (axiontag);
	Samples InCryoOther = LoadRun2bInCryoOther (axiontag);
	Samples dirt		= LoadRun2bDirt		(axiontag);
	Samples data		= LoadRun2bRHCData		(axiontag);
	Samples ext			= LoadRun2bEXT		(axiontag);


	double PlotPOT = data.GetPOT();

	// PRECUT --------------------------------------------------------------------------------
	TString Label = "AxionRun2bRHC"+axiontag+"_2s0t_twoMassPoints";
	TString Precut = "(reco_asso_tracks == 0 && reco_asso_showers == 2)";
	Precut +="&& reco_vertex_dist_to_SCB > 2";
//	Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003CosmicClassifier_mva < 0.25";//Non-signal regon
//	Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003PionClassifier_mva < 0.3";//signal regon
//	Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003NueCCClassifier_mva < 0.5";//Non-signal regon
	//Optimized for NueCC
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003PionClassifier_mva < 0.5";//Non-signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003NueCCClassifier_mva < 0.3";//Non-signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003CosmicClassifier_mva > 0.7";//Non-signal regon

	//Optimized for Pi0
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003PionClassifier_mva < 0.3";//Non-signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003NueCCClassifier_mva < 0.5";//Non-signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003CosmicClassifier_mva > 0.9";//Non-signal regon

	//Optimized for axions003
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003PionClassifier_mva > 0.98";//signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003NueCCClassifier_mva > 0.97";//signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003CosmicClassifier_mva > 0.94";//signal regon
	//		Precut +="&& NuMIRHCRun2bPre16880AxionV3ma003AxionLikeClassifier_mva > 0.5";//Non-signal regon
